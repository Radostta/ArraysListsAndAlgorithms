...
namespace CommandInterpreter
{
    class CommandInterpreter
    {
        ...
        private static void ExecuteRollRightCommand(string[] commandArgs, List<string> collection)
        {
            ...
            int numberOfRolls = count % collection.Count;

            var elementsToRotate = collection.Skip(collection.Count - numberOfRolls).Take(numberOfRolls).ToArray();

            collection.InsertRange(0, elementsToRotate);
